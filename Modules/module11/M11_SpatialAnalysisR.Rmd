...
dist_df <- data.frame(landfill_near = dist)

# combine with centroid object
library(units)
#install.packages("units)
# drop_units removes the unit ('m') from the distance value in each cell
tract_fill_dist <- cbind(erie_centroids, drop_units(dist_df))
...
st_crs(erie_centroids)

st_crs(landfill_near)

landfill_near_crs <- landfill_near

landfill_near_crs <- st_crs(landfill_near)

st_crs(landfill_near_crs)
```
...
-   So much information on [ggplot](https://ggplot2.tidyverse.org/). cant't cover here
